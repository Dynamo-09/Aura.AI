...
async def get_stock_data(stock_name: str):
    try:
        ...
        # 3. AI Predictions (Numpy-based to fit in Vercel 250MB limit)
        prediction_data = []
        predictions = []
        if 'Close_Price' in df_main.columns and not df_main['Close_Price'].isnull().all():
            ...
            if len(y) > 10:
                # Use a lightweight momentum and volatility model using pure numpy
                ...
                np.random.seed(int(current_price * 100) % 10000) # Deterministic but varied
                for i in range(30):
                    # Add some noise to make it look realistic
                    noise = np.random.normal(0, volatility * 0.2)
                    # Dampen momentum over time to prevent extreme predictions
                    dampened_momentum = momentum * (0.9 ** i)
                    current_price = current_price + dampened_momentum + noise
